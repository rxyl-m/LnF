...
function initSplash() {
    ...
    // Coming from an internal page link? Skip splash entirely — let the
    // CSS pageEnter animation on .app-shell handle the transition instead.
    if (sessionStorage.getItem('lf_navigating')) {
        sessionStorage.removeItem('lf_navigating');
        splash.remove();
        return;
    }

    // Fresh load (direct URL / refresh / first visit) — show full splash.
    splash.innerHTML = `
...
function initPageTransition() {
    document.addEventListener('click', e => {
        ...
        // Skip: anchors, external URLs, javascript:, mailto:, empty
        ...
        e.preventDefault();
        // Flag so the next page skips the splash
        sessionStorage.setItem('lf_navigating', '1');
        // Trigger exit animation, then navigate after it finishes
